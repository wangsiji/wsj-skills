...
- [ ] 当前备份已留存
- [ ] 用户权限边界明确（**只有「新建一个全新的 skill」才须批准；已有 skill 内部的文件增/删/改——包括在 `references/` 新建 `.md` 文档、改 `SKILL.md`、删文件——都不须批准**。本次会话已踩坑：曾把"在已有 skill 内新建文档"误当"新建 skill"去求批准，用户纠正）
...
- ❌ 删有效经验资产只为了压行数
- ❌ 未经用户明确批准新建 skill（含自改进机制自动沉淀的）；误判"不存在"——核查 skill 须全盘 `find ~/.hermes/skills`，不只 `custom/`。⚠️ 注意区分：**"新建 skill"** 才需批准；**"在已有 skill 内部新建/修改/删除文件"（如加 `references/xxx.md`）不需要批准**——这是用户 2026-07-19 明确纠正过的边界，勿把"文件级改动"误判为"skill 级改动"去求批准
