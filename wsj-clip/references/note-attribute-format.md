...
剪藏原文 下的所有正文标题必须统一降级一级（加一个 #：原文 H2→H3、H3→H4），从 H3 开始排列，避免与 `## 剪藏原文` / `## AI 智能总结` 的 H2 级别冲突。代码块内的 `#` 注释不降级。

**表格规则**：每一张 markdown 表格的表头行前必须有一个空行（GFM 渲染要求，否则表格不显示）。merge-clip.py 的 `ensure_table_blank_before()` 会自动补齐，无需手动处理。
